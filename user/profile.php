...
<head>
    ...
    <style>
        body {
            margin: 0;
            padding: 0;
            background-image: url('https://www.schemecolor.com/wallpaper?i=75160&desktop'); /* Replace 'background.jpg' with the path to your background image */
...
<div class="container">
    ...
    <h2 style="color: white;"><u>User account Details</u></h2>
                                  <?php
                               include('../includes/config.php'); 
                                include('../includes/session.php'); 
                                $place="SELECT * FROM `tbl_user` WHERE `category`='user' AND `id`='$Uid'";
                                $place_run=mysqli_query($conn,$place);
                                        ...
                                        while ($row=mysqli_fetch_assoc($place_run))
                                         {  
                                            ...
                                            echo "<p style='color:  white;'>User: " . $row['user_type'] . "</p>";
...
$_password="";
    ...
    $id=$_GET['up_id']; 
    $sql="SELECT * from `tbl_user` WHERE  `category`='user' AND  `id`='$id'";
